...
### 个人云同步
- 登录支持 `personalCloudSnapshots` 的自建服务器后，同步全部本地个人会话与点名记录，包括已关闭会话和删除 tombstone
- 使用账户隔离、内容校验和与 revision 条件写入；两端同时修改时不会静默覆盖，而是要求明确选择保留本机或云端
- 从云端恢复只替换个人会话与日志，不修改词库、设置、QTH 历史或协作会话
